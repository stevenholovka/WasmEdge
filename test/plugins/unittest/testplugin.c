...
static const WasmEdge_String NameStringDefaultValue = { .Buf = NameCString, .Length = 4};

WasmEdge_Result HostFuncAdd(void *Data __attribute__((unused)),
                            const WasmEdge_CallingFrameContext *CallFrameCxt __attribute__((unused)),
...
WasmEdge_ModuleInstanceContext *CreateTestModule(const struct WasmEdge_ModuleDescriptor *Desc __attribute__((unused))) {
  WasmEdge_String ModuleName = WasmEdge_StringCreateByCString("wasmedge_plugintest_c_module");
